...
```<form action="" method="">```. Предлагаю сразу вписать туда значения, что бы мы привыкали к заполнению нужных атрибутов. В action мы поставим заглушку **#**, решётка часто используется заглушки ссылки. А в метод передачи данных мы напишем **post**. Теоретически там можно написать и **get**. Это то как будут передаваться наши данные. Подробнее о HTTP и методах HTTP можно почитать по ссылкам внизу статьи. Результат:
